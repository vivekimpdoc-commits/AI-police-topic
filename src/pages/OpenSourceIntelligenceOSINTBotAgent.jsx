...
const OpenSourceIntelligenceOSINTBotAgent = () => {
  const navigate = useNavigate();
  const [activeTab, setActiveTab] = useState('overview');
  
  const [messages, setMessages] = useState([
    { sender: 'ai', text: 'Open Source Intelligence O S I N T Bot AI Engine Online. Systems synchronized. Ready for queries.' }
  ]);
  ...
  const handleSendMessage = (text) => {
    ...
    setTimeout(() => {
      ...
      if (lowerText.includes("scan") || lowerText.includes("analyze")) {
        ...
        response = { text: "I am the Open Source Intelligence O S I N T Bot AI. I automate complex workflows, provide real-time analytics, and ensure 100% compliance. How can I assist you further?" };
      }
  ...
  const renderContent = () => {
    switch (activeTab) {
      case 'overview':
        return (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="overview-panel premium-module-panel" style={{ background: 'rgba(15,23,42,0.8)', border: '1px solid #3b82f666', borderRadius: '16px', padding: '3rem', position: 'relative', overflow: 'hidden' }}>
            <div style={{ position: 'absolute', top: '-10%', left: '-5%', width: '300px', height: '300px', background: 'radial-gradient(circle, rgba(59,130,246,0.2) 0%, transparent 70%)', zIndex: 0 }}></div>
            ...
            <div style={{ position: 'relative', zIndex: 1, display: 'flex', gap: '3rem', alignItems: 'center' }}>
              <div style={{ width: '400px', height: '300px', display: 'flex', justifyContent: 'center', alignItems: 'center', position: 'relative' }}>
                <motion.div animate={{ rotate: 360 }} transition={{ duration: 30, repeat: Infinity, ease: "linear" }} style={{ position: 'absolute', width: '250px', height: '250px', border: '2px dashed #3b82f666', borderRadius: '50%' }} />
                <motion.div animate={{ rotate: -360 }} transition={{ duration: 25, repeat: Infinity, ease: "linear" }} style={{ position: 'absolute', width: '200px', height: '200px', border: '2px solid #3b82f633', borderRadius: '50%' }} />
                <div style={{ width: '120px', height: '120px', background: 'rgba(15,23,42,0.9)', border: '2px solid #3b82f6', borderRadius: '50%', display: 'flex', justifyContent: 'center', alignItems: 'center', boxShadow: '0 0 30px #3b82f680', zIndex: 2 }}>
                  <Cpu size={50} color="#3b82f6" />
                </div>
              </div>
              <div style={{ flex: 1 }}>
                <h2 style={{ fontSize: '2.5rem', color: '#fff', marginBottom: '1rem', textShadow: '0 0 20px #3b82f680' }}>Open Source Intelligence O S I N T Bot AI</h2>
                <p style={{ fontSize: '1.2rem', color: '#cbd5e1', lineHeight: '1.8', marginBottom: '2rem' }}>
                  The Open Source Intelligence O S I N T Bot AI provides enterprise-grade automation and predictive analytics. It seamlessly integrates into existing workflows to eliminate manual bottlenecks, detect anomalies in real-time, and enforce strict regulatory compliance.
                </p>
                <div style={{ display: 'flex', gap: '1rem' }}>
                  <div style={{ background: 'rgba(59,130,246,0.2)', borderLeft: '4px solid #3b82f6', padding: '1rem', borderRadius: '0 8px 8px 0' }}>
                     <h4 style={{ color: '#93c5fd', margin: '0 0 0.5rem 0' }}>Real-time Intelligence</h4>
      ...
      case 'dashboard':
        return (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="overview-panel">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', borderBottom: '1px solid #3b82f666', paddingBottom: '1rem' }}>
              <h2 style={{ color: '#93c5fd', margin: 0, display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
                <Activity className="inline-icon" size={28} /> SYSTEM DASHBOARD
              </h2>
              <button onClick={handleGenerateReport} style={{ background: '#3b82f6', color: '#fff', border: 'none', padding: '0.6rem 1.2rem', borderRadius: '8px', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
                <Download size={18} /> Export Analytics
              </button>
            </div>
            
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem', marginBottom: '2rem' }}>
              <div style={{ background: 'rgba(15,23,42,0.8)', border: '1px solid #3b82f666', borderRadius: '12px', padding: '1.5rem', position: 'relative' }}>
                ...
                <p style={{ color: '#93c5fd', fontSize: '0.85rem', marginTop: '0.5rem' }}>Operational Efficiency: 85%</p>
              </div>
            ...
            <div className="panel" style={{ padding: '2rem', border: '1px solid #3b82f666' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                ...
                <div>
                  ...
                  <div style={{ height: '8px', background: 'rgba(255,255,255,0.1)', borderRadius: '4px' }}>
                    <motion.div initial={{ width: 0 }} animate={{ width: '65%' }} transition={{ duration: 1 }} style={{ height: '100%', background: '#3b82f6' }} />
                  </div>
                </div>
              </div>
            </div>
            ...
            <AnimatePresence>
              {showReportModal && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.8)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <div style={{ background: '#0f172a', border: '1px solid #3b82f6', borderRadius: '16px', padding: '2rem', width: '90%', maxWidth: '500px' }}>
                    <h3 style={{ color: '#93c5fd', marginTop: 0, marginBottom: '1.5rem' }}>Generating Report...</h3>
                    ...
                    {reportStep >= 4 && (
                      <button onClick={() => setShowReportModal(false)} style={{ width: '100%', background: '#3b82f6', color: '#fff', border: 'none', padding: '0.8rem', borderRadius: '8px', cursor: 'pointer' }}>
                        Download PDF
                      </button>
                    )}
                  </div>
      ...
      case 'ledger':
        return (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="overview-panel" style={{ border: '1px solid #3b82f666' }}>
            <h2 style={{ color: '#93c5fd', marginBottom: '1.5rem', borderBottom: '1px solid #3b82f666', paddingBottom: '1rem' }}>
              DATA LEDGER
            </h2>
            <div style={{ background: 'rgba(15,23,42,0.8)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', overflow: 'hidden' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', color: '#e2e8f0' }}>
                <thead>
                  <tr style={{ background: 'rgba(59,130,246,0.2)', textAlign: 'left', borderBottom: '1px solid #3b82f666' }}>
                    <th style={{ padding: '1rem', color: '#93c5fd' }}>ID Reference</th>
                    <th style={{ padding: '1rem', color: '#93c5fd' }}>Category</th>
                    <th style={{ padding: '1rem', color: '#93c5fd' }}>AI Confidence</th>
                    <th style={{ padding: '1rem', color: '#93c5fd' }}>Status</th>
                  </tr>
                </thead>
                <tbody>
      ...
      case 'demo':
        return (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="overview-panel" style={{ height: '600px', display: 'flex', flexDirection: 'column', border: '1px solid #3b82f666' }}>
            <div style={{ background: 'rgba(15,23,42,0.9)', border: '1px solid #3b82f666', borderRadius: '16px', display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
              <div style={{ padding: '1rem 1.5rem', borderBottom: '1px solid rgba(255,255,255,0.05)', background: 'rgba(59,130,246,0.2)' }}>
                <h3 style={{ margin: 0, color: '#e2e8f0', fontSize: '1.1rem' }}>Interactive AI Terminal</h3>
              </div>
              <div style={{ flex: 1, overflowY: 'auto', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                {messages.map((msg, idx) => (
                  <div key={idx} style={{ display: 'flex', justifyContent: msg.sender === 'user' ? 'flex-end' : 'flex-start' }}>
                    <div style={{ maxWidth: '80%', padding: '1rem', borderRadius: '12px', background: msg.sender === 'user' ? 'rgba(59,130,246,0.2)' : 'rgba(255,255,255,0.05)', border: `1px solid ${msg.sender === 'user' ? '#3b82f666' : 'rgba(255,255,255,0.1)'}`, color: '#f8fafc' }}>
                      <div style={{ color: msg.sender === 'user' ? '#93c5fd' : '#3b82f6', fontSize: '0.85rem', fontWeight: 'bold', marginBottom: '0.5rem' }}>
                        {msg.sender === 'user' ? 'Operator' : 'Open Source Intelligence O S I N T Bot AI'}
                      </div>
                      <div>{msg.text}</div>
                ...
                {isTyping && <div style={{ color: '#3b82f6' }}>AI is typing...</div>}
                <div ref={chatEndRef} />
              </div>
              <div style={{ padding: '1.5rem', borderTop: '1px solid rgba(255,255,255,0.05)', background: 'rgba(0,0,0,0.2)' }}>
                <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                  ...
                  <button onClick={() => handleSendMessage(inputValue)} style={{ background: '#3b82f6', color: '#fff', border: 'none', borderRadius: '12px', padding: '0 1.5rem', height: '50px', cursor: 'pointer', fontWeight: 'bold' }}>Send</button>
                </div>
              </div>
            </div>
          </motion.div>
      ...
      case 'features':
        return (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="overview-panel" style={{ border: '1px solid #3b82f666' }}>
            <h2 style={{ color: '#93c5fd', marginBottom: '2rem' }}>AI Features</h2>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem' }}>
              <div className="feature-card" style={{ background: 'rgba(15,23,42,0.8)', padding: '2rem', borderRadius: '12px', border: '1px solid #3b82f633' }}>
      ...
      case 'use-cases':
        return (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="overview-panel" style={{ border: '1px solid #3b82f666' }}>
            <h2 style={{ color: '#93c5fd', marginBottom: '2rem' }}>Scenarios</h2>
      ...
      case 'architecture':
        return (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="overview-panel" style={{ border: '1px solid #3b82f666' }}>
            <h2 style={{ color: '#93c5fd', marginBottom: '2rem' }}>System Architecture</h2>
            <div style={{ background: 'rgba(15,23,42,0.8)', padding: '2rem', borderRadius: '12px', border: '1px solid #3b82f633', textAlign: 'center' }}>
               <div style={{ display: 'flex', justifyContent: 'space-around', alignItems: 'center', flexWrap: 'wrap', gap: '2rem' }}>
                  ...
                  <div style={{ width: '250px' }}><div style={{ background: 'rgba(59,130,246,0.2)', border: '2px solid #3b82f6', padding: '2rem', borderRadius: '12px' }}><h3 style={{color:'#fff', margin:0}}>AI Core</h3></div></div>
                  <div style={{ color: '#3b82f6' }}>→</div>
      ...
      case 'tech-stack':
        return (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="overview-panel" style={{ border: '1px solid #3b82f666' }}>
            <h2 style={{ color: '#93c5fd', marginBottom: '2rem' }}>Technologies Used</h2>
      ...
      case 'implementation':
        return (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="overview-panel" style={{ border: '1px solid #3b82f666' }}>
            <h2 style={{ color: '#93c5fd', marginBottom: '2rem' }}>Code Implementation</h2>
...
def process_data(input_stream):
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#020617', color: '#f8fafc', fontFamily: "'Inter', sans-serif", padding: '2rem' }}>
      <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'radial-gradient(circle at 50% 80%, rgba(59,130,246,0.2), transparent 40%)', zIndex: 0, pointerEvents: 'none' }}></div>
      <div style={{ position: 'relative', zIndex: 1, maxWidth: '1400px', margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '3rem', borderBottom: '1px solid #3b82f633', paddingBottom: '1.5rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
            <button onClick={() => navigate(-1)} style={{ background: 'rgba(15, 23, 42, 0.8)', border: '1px solid #3b82f666', color: '#93c5fd', padding: '0.8rem', borderRadius: '10px', cursor: 'pointer' }}><ArrowLeft size={20} /></button>
            <div>
              <h1 style={{ margin: 0, fontSize: '2.5rem', color: '#fff', textTransform: 'uppercase' }}>OPEN SOURCE INTELLIGENCE O S I N T BOT AI</h1>
              <div style={{ marginTop: '0.5rem', color: '#93c5fd', fontSize: '0.9rem' }}>INTELLIGENCE CORE ACTIVE</div>
            </div>
          </div>
        </div>
        <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem', flexWrap: 'wrap' }}>
          {tabs.map((tab) => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{ background: activeTab === tab.id ? 'rgba(59,130,246,0.2)' : 'rgba(15,23,42,0.6)', border: `1px solid ${activeTab === tab.id ? '#3b82f6' : 'rgba(255,255,255,0.1)'}`, color: activeTab === tab.id ? '#93c5fd' : '#94a3b8', padding: '0.8rem 1.2rem', borderRadius: '8px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', fontWeight: 'bold' }}>
              {tab.icon} {tab.label}
            </button>
          ))}
        </div>
...
export default OpenSourceIntelligenceOSINTBotAgent;
