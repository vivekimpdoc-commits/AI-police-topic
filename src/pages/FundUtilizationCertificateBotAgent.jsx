...
const FundUtilizationCertificateBotAgent = () => {
  ...
  const [messages, setMessages] = useState([
    { sender: 'ai', text: 'Fund Utilization Certificate System Online. Ready for operational queries.' }
  ]);
      ...
      case 'overview':
        return (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="overview-panel premium-module-panel" style={{ background: 'rgba(15,23,42,0.8)', border: '1px solid rgba(16, 185, 129, 0.4)', borderRadius: '16px', padding: '3rem', position: 'relative', overflow: 'hidden' }}>
            <h2 style={{ fontSize: '2.5rem', color: '#fff', marginBottom: '1rem' }}>Fund Utilization Certificate <span style={{ color: '#10b981' }}>AI</span></h2>
            <p style={{ fontSize: '1.2rem', color: '#cbd5e1', lineHeight: '1.8' }}>
              The Fund Utilization Certificate Bot is designed to automate and optimize workflows, providing real-time analytics, predictive modeling, and absolute transparency across administrative and financial protocols.
            </p>
          </motion.div>
        );
      case 'dashboard':
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto', position: 'relative', zIndex: 1 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', marginBottom: '3rem' }}>
          ...
          <h1 style={{ margin: 0, fontSize: '2.5rem', color: '#fff', textTransform: 'uppercase', letterSpacing: '2px' }}>
            FUND UTILIZATION CERTIFICATE <span style={{ color: '#10b981' }}>AI</span>
          </h1>
        </div>
