...
import { 
  ...
  Workflow, Globe, Search, Fingerprint, FileSearch
} from 'lucide-react';
...
const InvestigationLanding = () => {
  ...
  const prismaSchemaCode = `// Autonomous Investigation Assistant - Schema
generator client {
...
enum CaseStatus { OPEN, PENDING_REVIEW, CLOSED, REOPENED }
...
model EvidenceItem {
  ...
  type            String   // IMAGE, VIDEO, AUDIO, TEXT, PHYSICAL
  fileUrl         String?
  aiAnalysisResult Json?   // Stores AI extraction like OCR, Facial recognition tags
  chainOfCustody  Json?    // History of access
  createdAt       DateTime @default(now())
}`;
  ...
  return (
    <div style={{ background: 'var(--color-bg-dark)', minHeight: '100vh', color: '#fff' }}>
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            ...
            <h1 style={{ fontSize: '3.5rem', fontWeight: '800', lineHeight: '1.2', marginBottom: '1.5rem', textShadow: '0 0 20px rgba(6,182,212,0.3)' }}>
              Enterprise AI Autonomous Investigation Assistant
            </h1>
            <p style={{ fontSize: '1.2rem', color: '#cbd5e1', maxWidth: '800px', lineHeight: '1.8' }}>
              A modern enterprise investigation workspace. Uses AI to analyze evidence, extract text via OCR, generate timelines, and identify suspects using multimodal machine learning models.
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <AnimatedSection>
        <SectionHeading title="Technology Stack" subtitle="AI Vision and Text analysis backend architecture." icon={Layers} />
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: '1.5rem', marginTop: '2rem' }}>
          <TechBadge icon={Cpu} name="OpenCV & PyTorch" type="Computer Vision AI" />
          <TechBadge icon={FileSearch} name="Tesseract & AWS Textract" type="OCR Processing" />
          <TechBadge icon={Database} name="Pinecone / pgvector" type="Vector Database" />
          <TechBadge icon={Globe} name="React & Framer Motion" type="Frontend Workspace" />
        </div>
      </AnimatedSection>

      <AnimatedSection style={{ background: 'rgba(8,17,33,0.3)' }}>
        <SectionHeading title="Core Features" subtitle="AI modules supporting the investigator." icon={Workflow} />
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginTop: '2rem' }}>
          <div style={{ background: 'rgba(15,23,42,0.5)', border: '1px solid rgba(255,255,255,0.1)', padding: '1.5rem', borderRadius: '12px' }}>
            <h3 style={{ color: '#06b6d4', margin: '0 0 0.5rem 0', fontSize: '1.1rem' }}>Automated Evidence Extraction</h3>
            <p style={{ margin: 0, color: '#94a3b8', lineHeight: '1.6' }}>Automatically processes uploaded images and PDFs to extract names, dates, and vehicle license plates.</p>
          </div>
          <div style={{ background: 'rgba(15,23,42,0.5)', border: '1px solid rgba(255,255,255,0.1)', padding: '1.5rem', borderRadius: '12px' }}>
            <h3 style={{ color: '#06b6d4', margin: '0 0 0.5rem 0', fontSize: '1.1rem' }}>AI Timeline Generation</h3>
            <p style={{ margin: 0, color: '#94a3b8', lineHeight: '1.6' }}>Correlates events across hundreds of documents to automatically generate a suspect timeline visualization.</p>
          </div>
        </div>
      </AnimatedSection>

      <AnimatedSection>
        <SectionHeading title="Database Architecture" subtitle="Schema for Cases and Evidence." icon={Database} />
